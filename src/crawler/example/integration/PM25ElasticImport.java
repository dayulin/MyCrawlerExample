...
/**
 * 整合練習：PM2.5 資料取得 + ElasticSearch 輸入
 *
 * @author Abola Lee
 */
public class PM25ElasticImport {
	...
	public static void main(String[] args) {
		...
		for (Element elem : jsoupDoc.select("Data")) {
			...
			String majorpollutant = elem.getElementsByTag("MajorPollutant").text();
			...
			String publishtime = elem.getElementsByTag("PublishTime").text().replace(' ', 'T') + ":00+0800";
			...
			String windspeed = elem.getElementsByTag("WindSpeed").text();
			Double winddirec = Doubles.tryParse(elem.getElementsByTag("WindDirec").text());

			// Elasticsearch data format
			String elasticJson = "{" + "\"co\":" + co + ",\"county\":\"" + county + "\"" + ",\"fpmi\":" + fpmi
					+ ",\"majorpollutant\":\"" + majorpollutant + "\"" + ",\"no\":" + no + ",\"no2\":" + no2
					+ ",\"nox\":" + nox + ",\"o3\":" + o3 + ",\"pm10\":" + pm10 + ",\"pm25\":" + pm25 + ",\"psi\":"
					+ psi + ",\"publishtime\":\"" + publishtime + "\"" + ",\"sitename\":\"" + sitename + "\""
					+ ",\"so2\":" + so2 + ",\"status\":\"" + status + "\"" + ",\"windspeed\":\"" + windspeed + "\""
					+ ",\"winddirec\":" + winddirec + "}";

			System.out.println(
					// curl -XPOST http://localhost:9200/pm25/data -d '{...}'
					sendPost("http://" + elasticHost + ":" + elasticPort + "/" + elasticIndex + "/" + elasticIndexType,
							elasticJson));
		}
	}
